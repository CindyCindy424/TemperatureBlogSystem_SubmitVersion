<template>
<div id="Home">
    <img :class="['home-Background']" src="/static/images/banner.jpg">
    ...
    <div class="hottest-article" style="top: 600px; left: 50%; margin-left: -713px;">
        ...
        <div id="article-list">
            <div class="left-aside-list-item" v-for="(item,index) in HottestArticle" :key="index">
    ...
    <div class="latest-article" style="top: 1040px;">
        ...
        <div id="article-list">
            <div class="left-aside-list-item" v-for="(item,index) in LatestArticle" :key="index">
                <router-link :to="{path: '/ViewArticle', query:{ViewArticleNickname:hostNickName, ViewArticleUsername:myNickName, ViewArticleTitle:item.title}}">
                    <div style="color: #000000; text-overflow: ellipsis; overflow: hidden;">{{item.title}}</div>
                </router-link>
            </div>
    ...
    <div class="latest-topic" style="height: 344px;">
        ...
        <div id="article-list">
            <div class="left-aside-list-item" v-for="(item,index) in LatestTopic" :key="index">
                {{item.TopicTitle}}
            </div>
        </div>
    </div>
    ...
    <div id="h-album-list" style="position: absolute; top: 530px; left: 50%; margin-left: -270px; height: 120px; width: 800px;">
        <el-carousel :interval="5000" type="card" height="300px" width="200px" v-if="items.length === 0">
            <el-carousel-item  v-for="item in items" :key="item">
                <h3 class="medium">{{ item }}</h3>
                <img src="/static/images/noalbum.jpg" class="image" style="cursor: pointer; height: 100; width 100%;" object-fit="contain" />
            </el-carousel-item>
        </el-carousel>
        <el-carousel :interval="5000" type="card" height="300px" width="200px" v-if="items.length !== 0">
            <el-carousel-item v-for="item in items" :key="item">
    ...
    <div id="my-article-container">&nbsp;&#160;&nbsp;&#160;我的文章（{{myarticlenum}}）
        <router-link :to="{path: '/MyArticle', query:{MyArticleid:hostId,MyArticlename:hostNickName,MyArticleUserid:myId,MyArticleusername:myNickName}}">
  ...
  methods: {
    ...
    GUIsuccessfully () {
      if (this.GUIajax.readyState === 4 && this.GUIajax.status === 200) {
        ...
        this.myNickName = JSON.parse(this.GUIajax.responseText).userInfo.nickName
    ...
    getHostUserInfo () {
      ...
      this.GUIajax.open('POST', 'http://139.224.255.43:7779/Account/getUserInfoByID?user_id=' + id, true)
      this.GUIajax.setRequestHeader('Authorization', 'Bearer ' + headerToken)
      this.GUIajax.onreadystatechange = this.GHUIsuccessfully
      this.GUIajax.send()
    },
    GHUIsuccessfully () {
      if (this.GUIajax.readyState === 4 && this.GUIajax.status === 200) {
