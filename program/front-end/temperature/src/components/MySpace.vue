<template>
    <div :load='this.test()' style="background-color:#FDFDFD;height:800px;">
        <img :class="['mySpaceBackground']" src="/static/images/newbanner.jpg">
        <div style="margin:auto 0;">
          <el-button :class="['sideBar1']" v-on:click="switchToAccountInfo()">
            <p style="position:relative;top:-11px;left:35px;">账号信息</p>
          </el-button>
          <el-button :class="['sideBar2']" v-on:click="switchToStar()">
            <p style="position:relative;top:-11px;left:35px;">我的关注</p>
          </el-button>
          <el-button :class="['sideBar3']" v-on:click="switchToFans()">
            <p style="position:relative;top:-11px;left:35px;">我的粉丝</p>
          </el-button>
          <el-card class="box-card" :class="['sideBar4']">
          </el-card>
...
Vue.component('AccountInfo', {
  ...
  template: `
   <div :load="this.getAllInfo()">
      ...
      <el-button v-if="this.isChangeAnnouncement===false" type="success" size="small" style="position:absolute;left:650px;top:220px;" @click="this.changeAnnouncement">修改公告</el-button>
      <el-input v-if="this.isChangeAnnouncement===true" v-model="newAnnouncement" placeholder="请输入内容" style="position:absolute;left:420px;top:217px;width:200px;"></el-input>
      <p :class="['word']" style="position:absolute;left:430px;top:200px;" v-text="this.userAnnouncement" v-if="this.isChangeAnnouncement===false"></p>
      <el-button size="small"  type="success" style="position:absolute;left:650px;top:220px;" v-on:click="newChangeAnnouncement()" v-if="isChangeAnnouncement==true">确认</el-button>
  ...
  methods: {
    ...
    changeAnnouncement: function () {
      this.isChangeAnnouncement = true
    },
    newChangeAnnouncement: function () {
      this.isChangeAnnouncement = false
      var xhr = new XMLHttpRequest()
      xhr.onreadystatechange = () => {
        if (xhr.readyState === 4 && xhr.status === 200) {
          this.getAllInfo()
      ...
      var headerToken = document.cookie.split(';')[0].split('=')[1]
      xhr.open('POST', 'http://139.224.255.43:7779/Account/createAnnouncementByNickName?nick_name=' + this.nickName + '&content=' + this.newAnnouncement)
      xhr.setRequestHeader('Authorization', 'Bearer ' + headerToken)
      xhr.send()
    },
