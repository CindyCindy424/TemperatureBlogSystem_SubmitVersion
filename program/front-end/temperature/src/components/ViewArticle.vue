...
<div id="ViewArticle-page">
    <img src="/static/images/newbanner.jpg" style="position:absolute;top:-40px;left:0px;width:100%;height:352px;">
    <div id="top-menu" v-if="isMenuShow">
        <button class="t-m-button" v-for="(item,index) in Favoritelist" :key="index" @click="collectArticle(item.favouriteName)">
            {{item.favouriteName}}
        </button>
    </div>
    <div class="ViewArticle-personal">
        ...
        <div>
            <button id="ViewArticle-focus" v-show="!isMyArticle" @click="ChangeFocusByname()">
                <div>{{isFollowed?"已关注":"+关注"}}</div>
            </button>
...
<span>私信</span>
            ...
            <!-- router to MyHome -->
            <router-link :to="{path: '/MyHome', query:{hostId:articleownerid, myId:accountownerid}}">
    ...
    <div class="ViewArticle-HowPost">
        ...
        <div id="ViewArticle-list2">
            <div class="ViewArticle-left-aside-list-item" v-for="(item,index) in HArticles" :key="index" @click="reflashpage(item.title,item.authorname)">
    ...
    <div id="ViewArticle-content-container">
        ...
        <div id="ViewArticle-bottom">
            ...
            <div id="ViewArticle-collect" @click="getFolderlist()" v-if="collectlimit===1">
...
{page: 3}
  ...
  computed: {
    isMyArticle: function () {
      ...
      return this.articleownername === this.accountownername
    }
  },
  created: function () {
    this.getQuery()
  ...
  methods: {
    ...
    topMenuToggle () {
      this.isMenuShow = this.isMenuShow === false
    },
    addComment () {
    ...
    //  console.log(JSON.parse(this.GUNajax.responseText).userInfo.nickName);
    //  console.log(JSON.parse(this.GUNajax.responseText).userInfo.nickName);
    //  console.log(JSON.parse(this.GUNajax.responseText).userInfo.nickName);
    //  console.log(JSON.parse(this.GUNajax.responseText).userInfo.nickName);
    //  console.log(JSON.parse(this.GUNajax.responseText).userInfo.nickName);
    ...
    // console.log(JSON.parse(this.GANajax.responseText).userInfo.nickName);
    // console.log(JSON.parse(this.GANajax.responseText).userInfo.nickName);
    // console.log(JSON.parse(this.GANajax.responseText).userInfo.nickName);
    // console.log(JSON.parse(this.GANajax.responseText).userInfo.nickName);
...
this.getComment();
    ...
    // }
    ChangeFocusByname () {
      var focusname = this.articleownername
      var fansname = this.accountownername
      var headerToken = this.token
      if (!this.isFollowed) {
        this.ajax_follow = new XMLHttpRequest()
        this.ajax_follow.open('POST', 'http://139.224.255.43:7779/Account/createFollowByNickNames?nameOfBlogger=' + focusname + '&nameOfFans=' + fansname, true)
        this.ajax_follow.setRequestHeader('Authorization', 'Bearer ' + headerToken)
        this.ajax_follow.onreadystatechange = this.fw
        this.ajax_follow.send()
      } else {
        this.ajax_dfollow = new XMLHttpRequest()
        this.ajax_dfollow.open('POST', 'http://139.224.255.43:7779/Account/deleteFollowByNickNames?nameOfBlogger=' + focusname + '&nameOfFans=' + fansname, true)
        this.ajax_dfollow.setRequestHeader('Authorization', 'Bearer ' + headerToken)
    ...
    checkfollow () {
      ...
      var focusname = this.articleownername
      var headerToken = this.token
      this.CFajax = new XMLHttpRequest()
      this.CFajax.open('POST', 'http://139.224.255.43:7779/Account/checkFollowByName?nameOfCurrentUser=' + fansname + '&nameOfCheckUser=' + focusname, true)
      this.CFajax.setRequestHeader('Authorization', 'Bearer ' + headerToken)
      this.CFajax.onreadystatechange = this.CFsuccessfully
      this.CFajax.send()
    ...
    CFsuccessfully () {
      if (this.CFajax.readyState === 4 && this.CFajax.status === 200) {
        var receive = JSON.parse(this.CFajax.responseText)
...
#ViewArticle-time-logo {
    ...
    background-image: url('/static/images/timelogo.jpg');
...
#ViewArticle-praise-logo {
    ...
    background-image: url('/static/images/hand.png');
    background-size: contain;
    background-repeat: no-repeat;
    background-position: center;
    border-radius: 0px 30px 30px 0px;
...
#ViewArticle-agree-logo {
    ...
    background-image: url('/static/images/点赞2.png');
    background-size: contain;
    background-repeat: no-repeat;
    background-position: center;
    border-radius: 0px 30px 30px 0px;
...
#ViewArticle-agreed-logo {
    ...
    background-image: url('/static/images/点赞3.png');
    background-size: contain;
    background-repeat: no-repeat;
    background-position: center;
    border-radius: 0px 30px 30px 0px;
...
#ViewArticle-collect-logo {
    ...
    left: -20px;
    top: 2px;
    background-image: url('/static/images/收藏2.png');
    background-size: contain;
    background-repeat: no-repeat;
    background-position: center;
    border-radius: 0px 30px 30px 0px;
...
#ViewArticle-collected-logo {
    ...
    left: -20px;
    top: 2px;
    background-image: url('/static/images/收藏3.png');
    background-size: contain;
    background-repeat: no-repeat;
    background-position: center;
    border-radius: 0px 30px 30px 0px;
