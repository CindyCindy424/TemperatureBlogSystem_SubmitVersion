...
<div id="topic-area-page" :class="{loading:isLoading}">
    <div id="ta-photo">
        <img src="/static/images/newbanner.jpg" style="width:100%;"/>
    </div>
    <div id="main">
        <div id="latest-topics">
            <div id="latest-topics-title">
                ...
                <router-link id="topics-search-button" :to="{path:'/SearchResult',query:{SearchContent:LTSearchInput,ArticleOrTopic:2,userId:myID}}"></router-link>
            </div>
            ...
            <div id="latest-topics-page-controller">
                ...
                <div :class="{'l-t-page-num':true,'l-t-page-num-selected':item.page===currentPage}" v-for="(item,index) in pageNums" :key="index" v-on:click="turnToPage(item.page)">
                    {{item.page}}
                </div>
        ...
        <div id="right-aside">
            ...
            <div id="my-topics">
                ...
                <div id="my-topics-list">
                    <div v-for="(item,index) in MTopics" class="right-aside-list-item MT-list-item" :key="index">
            ...
            <div id="hottest-topics">
                ...
                <div id="hottest-topics-list">
                    <div v-for="(item,index) in HTopics" class="right-aside-list-item" :key="index">
                        <router-link :to="{path:'/ViewTopic', query:{topicID:item.TopicId,userID:myID}}">{{item.TopicTitle}}</router-link>
                    </div>
                </div>
            </div>
  ...
  computed: {
    myID: function () {
      if (this.$route.query.userID === undefined) {
        this.$alert('this.$route.query.userID', '缺少值', {
  ...
  methods: {
    getZoneID () {
      if (this.$route.query.zoneID === undefined) {
        this.$alert('this.$route.query.zoneID', '缺少值', {
          confirmButtonText: '确定'
        })
      } else {
    ...
    getUTN () {
      if (this.ajax_getUserTopicNum.readyState === 4 && this.ajax_getUserTopicNum.status === 200) {
        ...
        if (receive.flag === 1) {
          ...
          this.$alert('getUserTopicNum失败', '请求错误', {
            confirmButtonText: '确定'
          })
        }
      }
    ...
    getANOU () {
      if (this.ajax_getAnswerNumOfUser.readyState === 4 && this.ajax_getAnswerNumOfUser.status === 200) {
        ...
        if (receive.flag === 1) {
          ...
          this.$alert('getAnswerNumOfUser失败', '请求错误', {
            confirmButtonText: '确定'
          })
        }
      }
    ...
    getMT () {
      if (this.ajax_getMyTopic.readyState === 4 && this.ajax_getMyTopic.status === 200) {
        ...
        if (receive.getTopicFlag === '1') {
          ...
          this.$alert('getMyTopicByPage失败', '请求错误', {
            confirmButtonText: '确定'
          })
        }
      }
    ...
    DT () {
      if (this.ajax_deleteTopic.readyState === 4 && this.ajax_deleteTopic.status === 200) {
        ...
        if (receive.deleteTopicAnswerFlag === '0') {
          ...
          this.$alert('deleteTopicByID失败', '请求错误', {
            confirmButtonText: '确定'
          })
        }
      }
...
.right-aside-list-item>span,
.right-aside-list-item>a {
    cursor: pointer;
}

.right-aside-list-item>span:hover,
.right-aside-list-item>a:hover {
    color: #B23535;
}
