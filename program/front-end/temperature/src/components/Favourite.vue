<template>
  <div id="yyw-favorite">
    <img id="yyw-banner" src="/static/images/newbanner.jpg" />
    ...
    <div id="yyw-left">
      ...
      <p style="position:absolute;left:50%;margin-left:-95px;font-size:24px;top:100px;" v-if="isFContent">您暂未创建收藏夹</p >
      ...
      <p style="font-size:30px; left:50%;margin-left:-140px;position:absolute;top:40px;color:grey" v-if="isContent">此收藏夹中无内容</p >
        <div class="yyw-f-item" v-for="item in favorite" :key="item.articleId">
          ...
          <span class="yyw-f-answerCount">
            <img class="yyw-readnum" src="/static/images/eye(2).png" />
            {{item.readNum}}
          </span>
  ...
  methods: {
    ...
    getFC: function () {
      if (
        ...
        if (this.favorite.length === 0) this.isContent = true
        else this.isContent = false
    ...
    getFFL: function () {
      if (
        ...
        if (this.favoriteList.length === 0) {
          this.isFContent = true
          this.favoriteName = ''
        } else this.isFContent = false
    ...
    confirmCreateIn: function () {
      ...
      this.getAjaxCreateFavorite.open(
        'POST',
        'http://139.224.255.43:7779/Favourite/createFolderByNickName?nick_name=' + this.currentUserName + '&folderName=' +
          this.cfavoriteName,
        true
      )
    ...
    confirmDeleteC: function () {
      ...
      this.getAjaxDeleteContent.open(
        'POST',
        'http://139.224.255.43:7779/Favourite/deleteArticleByID?nick_name=' + this.currentUserName + '&folderName=' +
          this.favoriteName +
          '&articleID=' +
          this.articleId,
        true
    ...
    getDC: function () {
      if (
        this.getAjaxDeleteContent.readyState === 4 &&
        this.getAjaxDeleteContent.status === 200
      ) {
        this.showFavoriteContent()
      }
    },
    confirmDeleteF: function () {
      ...
      this.getAjaxDeleteFavorite.open(
        'POST',
        'http://139.224.255.43:7779/Favourite/deleteFolderByName?nick_name=' + this.currentUserName + '&folderName=' +
          this.favoriteName,
        true
      )
