...
<div id="MyArticle-page">
    <div id="MyArticle-photo">
        <img src="/static/images/newbanner.jpg" alt="照片">
    </div>
    <div id="MyArticle-personal">
        ...
        <router-link :to="{path: '/MyHome', query:{hostId:userid, myId:accountownerid}}">
    ...
    <div id="MyArticle-search-container">
        <div id="MyArticle-search-box">
            <input id="MyArticle-search-info" placeholder="搜索我的文章..." v-model="inputshearch" type="text" />
            <div id="MyArticle-search-button" @click="this.pushToSearch"></div>
        </div>
    </div>
    <div class="MyArticle-hottest-article">
        ...
        <div id="MyArticle-article-list">
            <div class="MyArticle-left-aside-list-item" v-for="(item,index) in HottestArticle" :key="index">
                <router-link :to="{path:'/ViewArticle',query:{ViewArticleTitle:item.title,ViewArticleNickname:item.authorname,ViewArticleUsername:accountownername}}"><span id="MyArticle-hotarticlename">{{item.title}}</span></router-link>
            </div>
        </div>
    </div>
   ...
   <div class="left-aside-list-item" v-for="item in LatestComment">
    ...
    <div id="MyArticle-container">
        <div class="MyArticle-article-container" v-for="item in MyArticle" :key="item.article_id">
            <router-link :to="{path:'/ViewArticle',query:{ViewArticleTitle:item.title,ViewArticleNickname:item.User_name,ViewArticleUsername:accountownername}}">
            ...
            <span class="MyArticle-delete" @click="DeleteArticle(item.title)" v-if="ismyspace">删除</span>
        </div>
    </div>
    ...
    <footer id="MyArticle-footer">
        Copyright &copy;2020 Temperature team, All Rights Reserved.
    </footer>
</div>
</template>
...
{page: 3}
  ...
  computed: {
    ismyspace: function () {
      ...
      return this.userid === this.accountownerid
    }
  },
  created: function () {
    this.getQuery() // 初始化页面数据
    // if(this.nickname!=""){
    // this.getUserInfo();
    // }
    // if(this.userid!=0){
  ...
  methods: {
    ...
    getQuery: function () {
      ...
      this.accountownerid = this.$route.query.MyArticleuserid
      this.accountownername = this.$route.query.MyArticleusername
    },
    DeleteArticle (Articletitle) { // 删除文章
    ...
    pushToSearch: function () {
      this.$router.push({path: '/SearchResult', query: {SearchContent: this.inputshearch, ArticleOrTopic: 1, myId: this.accountownerid}})
    },
    // UserIdToname(){
    //  var id=this.userid;
    //  var headerToken=this.token;
...
.MyArticle-comment-logo {
    ...
    background-image: url('/static/images/评论.png');
    background-size: contain;
    background-repeat: no-repeat;
    background-position: center;
    border-radius: 0px 30px 30px 0px;
